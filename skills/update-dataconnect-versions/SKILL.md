---
name: update-dataconnect-versions
description: Automates updating the firebase-dataconnect emulator and firebase-tools version in CI, including creating a branch, committing the updates, pushing to GitHub, and creating a pull request.
---

# Update Dataconnect Versions

Use this skill to update the `firebase-dataconnect` module's emulator and `firebase-tools` dependencies to their latest versions. Follow these steps sequentially:

1. **Backup Versions JSON:**
   Copy `./firebase-dataconnect/gradleplugin/plugin/src/main/resources/com/google/firebase/dataconnect/gradle/plugin/DataConnectExecutableVersions.json` to `./firebase-dataconnect/gradleplugin/plugin/src/main/resources/com/google/firebase/dataconnect/gradle/plugin/DataConnectExecutableVersions.old.json`.

2. **Update Emulator Versions JSON:**
   Run the update script to update the versions JSON file `DataConnectExecutableVersions.json`. Use `run_shell_command` to execute: `./firebase-dataconnect/scripts/update_versions_json.sh`
...
7. **Fetch Latest firebase-tools Version:**
   Determine the latest release version number of the `firebase-tools` repository using the `gh` CLI or `get_latest_release` for `Firebase/firebase-tools`. Do not include the 'v' prefix in the version number (e.g., use `15.8.0` instead of `v15.8.0`).
...
10. **Push Changes to GitHub:**
   ...
   `git push -u origin Emulator_X_Y_Z:dconeybe/dataconnect/Emulator_X_Y_Z`
...
Follow the steps systematically and do not summarize output unprompted. Wait for completion at each step where needed before continuing. If any errors occur then abort.
