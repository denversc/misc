...
# mkdir -p ~/.config/tmux && (echo "set-option -g @tmux_power_theme \"moon\"" && echo "run-shell $PWD/tmux-power.sh") >> ~/.config/tmux/tmux.conf
...
RS="#[fg=$TC,bg=$G2]$larrow#[fg=$G0,bg=$TC] $date_icon $date_format #[fg=$G2]$larrow#[fg=$TC,bg=$G2] $time_icon $time_format "
...
tmux_set window-status-format         "#[fg=$G0,bg=$G2]$rarrow#[fg=$TC,bg=$G2] #I:#W #[fg=$G2,bg=$G0]$rarrow"
tmux_set window-status-current-format "#[fg=$G0,bg=$TC]$rarrow#[fg=$G0,bg=$TC,bold] #I:#W #[fg=$TC,bg=$G0,nobold]$rarrow"
